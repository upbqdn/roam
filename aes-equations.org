...
The actual occurrence of a 0-inversion during encryption or the key schedule is
...
[[.eqn-ro][equation ro]] and substitute \(c\) for \(b\). We obtain the
...
also lead to equations with higher than quadratic terms. There are, however,
22 linearly independent quadratic equations over \(\mathrm{GF}(2)\) in the
\(b_i\) and \(c_i\) variables, so the 20 equations we have derived are not
quite all of them. A further discussion can be found in [cite:@cid06 p. 77]. As also advised there,
...
[[file:aes.org::.eqn-rc][equation rc]] where the input is the
...
matrix from [[file:aes.org::.eqn-rc][equation rc]]. In order to describe the
...
operation and \(\mathbf{b^{-1}}\) to denote the output of its inversion
part --- the vector of the
...
*[[.fig-rt][Figure rt]]*. The state array of the \(\mathrm{SR}(n, 2, 2, e)\) cipher [cite:@cid05].
#+end_center
#+END_fig
...
MixColumns operation, let us rewrite [[.eqn-ro][equation ro]] into matrix
form:
...
\(x + 1\) and \(x\) into [[.eqn-r1][equation r1]], we get
...
[[file:aes.org::.eqn-rq][equation rq]], can then be expressed
by the matrix
...
\(\mathbf{b_{i-1}^{-1}}\) contains four components --- the outputs of the
S-box inversions --- each of which has four binary variables. It is straightforward to
...
\[\begin{pmatrix} k_{i,2q} \\ k_{i,2q+1} \end{pmatrix} = \begin{pmatrix} L_s k_{i-1,3}^{-1} \\ L_s k_{i-1,2}^{-1} \end{pmatrix} + \begin{pmatrix} 6_{16} \\ 6_{16} \end{pmatrix} + \begin{pmatrix} x^{i-1} \\ 0 \end{pmatrix} + \sum_{t=0}^{q} \begin{pmatrix} k_{i-1,2t} \\ k_{i-1,2t+1} \end{pmatrix}\]
...
Consider the four polynomials \(r_0 \oplus 1, r_1, r_2, r_3\) from [[.eqn-ro][equation ro]], together with the [[file:groebner-basis.org::.def-dn][field equations]] \(b_i^2 \oplus b_i\) and \(c_i^2 \oplus c_i\), as
polynomials in
...
\(\succeq_{\mathrm{grevlex}, \mathrm{grevlex}}\) on
\(\mathbb{F}[c_0, \ldots, c_3, b_0, \ldots, b_3]\) with
[[file:monomial-orders.org::.def-no][\(\succeq_{\mathrm{grevlex}}\)]] on both
...
functions of \(\mathbf{k_0}\), one function per bit of the ciphertext. We can
...
\(|\mathbf{k_0}| \cdot 2^{|\mathbf{k_0}|}\) operations even if we consider

...
The network has experienced almost no downtime since its inception. At the time
...
no central authority. Besides its near-perfect uptime, the network has suffered no
...
essentially what miners do. They repeatedly try enormous numbers of inputs as
...
inputs lead to the same output, or at least that it is impossible to find two
such inputs. We call such hash functions *collision-resistant*.
...
A legacy Bitcoin address is a Base58Check-encoded string of 26 to 34 characters,
...
that can be encoded into a human-readable format called a /seed phrase/
[cite:@bip0039], which consists of 12 English words that the user has to
retain. The wallet can then
...
Bitcoin cannot be considered private or untraceable since all of its
...
much money each address holds --- there are online blockchain explorers that
...
- *output* :: Contains the following:
  ...
  - *receiver's address* :: The receiver's public key, which serves as their Bitcoin address.
...
*[[.fig-bb][Figure bb]]*. Outline of transactions and their ordering [cite:@Nakamoto].
#+end_center
#+END_fig
...
Signature Algorithm/ (/ECDSA/) and, since the Taproot upgrade activated in
November 2021, also the /Schnorr signature algorithm/ for Taproot outputs. The
description of ECDSA can be found in [cite:@brown2009sec]. The algorithm employs the /secp256k1/
...
algorithm, and therefore effectively break ECDSA.
...
curve, together with the point at infinity serving as the identity element,
form an additive Abelian group under the standard point addition operation.
...
Bitcoin does not use encryption. It uses public-key cryptography solely for
digital signatures.
...
Turing-complete as it contains no instructions for jumps or loops. Such a design is
for security reasons --- it is not possible to create a malicious transaction
that would make the node get stuck in an infinite loop. [[.tab-bn][Table bn]]
...
- *time* :: A timestamp of when the block was created.
...
*[[.fig-br][Figure br]]*. The process of finding a valid Proof-of-Work.
#+end_center
#+END_fig
...
memoryless. Each hash produces a random number between \(0\) and \(2^{256}-1\), so
...
*[[.fig-bf][Figure bf]]*. The blockchain from the genesis block to the current tip.
#+end_center
#+END_fig
...
*[[.fig-bg][Figure bg]]*. A chain split.
#+end_center
#+END_fig
...
*[[.fig-b8][Figure b8]]*. The longest chain wins.
#+end_center
#+END_fig
...
*[[.fig-be][Figure be]]*. A rollback to the longer chain.
#+end_center
#+END_fig
...
maliciously and send deceptive messages, or send different messages to different
...
This attack is known as the /51% attack/. If an attacker has more than 50% of the
...
At the time of writing, the hash rate of the Bitcoin network is approximately
50 exahashes of SHA-256 per second, which is an unprecedented number. The energy consumption of Bitcoin
...
publishing an enormous number of them by sending bitcoin back to themselves. All
...
miners once the predetermined number of coins has entered circulation.
...
hash rate of an individual miner by having them submit /shares/ --- blocks whose
hashes fall below an easier, pool-defined target --- and counting how many they
submit. Each
...
hash that falls below not only the pool's easier target but the network target
itself. Also note that
...
while others do not, there can be a permanent chain split at the height where
the change took effect, in which case both groups end up with their own branch
of the original blockchain. Most likely, each group will give their chain a different
...
*[[.fig-bj][Figure bj]]*. A soft fork restricts the consensus rules.
#+end_center
#+END_fig
...
*[[.fig-bu][Figure bu]]*. A hard fork relaxes the consensus rules.
#+end_center
#+END_fig
...
For a Bitcoin light client, it is sufficient to keep only the block headers,
which contain the Merkle root hashes. If such a client needs to verify a transaction, it can link it to a
place in the chain via a Merkle branch obtained from a full node and check that other full nodes
...
able to process an arbitrary number of transactions over a certain period of
...
Every block can contain only a limited number of transactions. Based on this, it
...
and continue in their own way. In both cases, a new cryptocurrency is created since

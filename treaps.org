...
A treap is a data structure in the form of a self-balancing binary search tree,
which is suitable for storing ordered data more efficiently than a standard BST
[cite:@treaps].
...
property holds, i.e., every parent has a priority greater than or equal to that of its children.
...
rotation works as follows: consider a pointer to the current node, which has a
right child. We store the pointer to the right child in a temporary variable and
